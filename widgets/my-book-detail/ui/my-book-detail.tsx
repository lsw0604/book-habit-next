...
import { useMyBook } from '@/entities/my-book/lib/queries';
...
export default function MyBookDetail({ myBookId }: { myBookId: number }) {
  ...
  const datetime = dayjs(data?.book.datetime).format('YYYY-MM-DD');
  ...
  return (
    <div className="w-full h-auto border border-gray-300 rounded-lg shadow-lg bg-transparent px-2 py-4">
      ...
      <div className="w-full text-sm min-h-[140px] mt-2 h-auto">
        <div className="h-min-[140px] h-auto w-full flex items-center p-1 bg-gray-100 rounded-lg">
          {data.book.contents === '' ? (
