...
interface MyBookReadButtonProps {
  onClick: () => void;
  isActive?: boolean;
  disabled?: boolean;
  className?: string;
...
export function MyBookReadButton({
  onClick,
  isActive = false,
  disabled,
  className,
}: MyBookReadButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <BookCheck size={22} className="mb-1.5" />
      <span className="text-xs font-bold">읽음</span>
    </button>
  );
}
