import MyBookCommentEmpty from './my-book-comment-empty';
import MyBookCommentItem from './my-book-comment-item';
...
MyBookCommentList.Loader = function () {
  return (
    <div className="w-full">
      <ul className="grid grid-flow-col auto-cols-[100%] gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-none">
        {Array.from({ length: 5 }).map((_, index) => (
          <MyBookCommentItem.Loader key={`comment-item-loader-${index}`} />
        ))}
      </ul>
    </div>
  );
